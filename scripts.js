...
let alarmCount = 0;
let alarmTime;
let ring = new Audio("audio/alarm.mp3");
    ...
    var dname = now.getDay(),
        ...
        for(let i=0; i<alarmListArr.length;i++){
            if(alarmListArr[i]==`${hou.pad(2)}:${min.pad(2)}:${sec.pad(2)} ${pe}`){
                ...
                ring.load();
                ring.play();
                document.querySelector("#stopAlarmButton ").style.visibility= "visible";
            }
        }
}
...
function setAlarm(){
    ...
    if(time.includes("setHour") || time.includes("setMinute") || time.includes("AM/PM")){
        ...
        alarmCount++;
        document.querySelector(".alarm-list-container").innerHTML += `
        <div class="alarm-log" id="alarm${alarmCount}">
...
function stopAlarm(){
    ring.pause();
    document.querySelector("#stopAlarmButton").style.visibility= "hidden";
}
